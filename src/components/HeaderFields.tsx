...
import { ReportHeader, ReportKind } from "@/lib/types";
    ...
    <div className="flex flex-col gap-3 border-b-[2.5px] border-black pb-4 print:gap-1 print:border-b print:pb-1.5">
      <div className="flex items-start justify-between gap-4">
        <button
          ...
          title={open ? t("header.collapse") : t("header.expand")}
        >
          <h1 className="font-display text-2xl uppercase tracking-wide sm:text-3xl">{t(`reportTitle.${kind}`)} *</h1>
          <ChevronIcon open={open} />
        </button>
        <h1 className="hidden font-display text-base uppercase tracking-wide print:block">{t(`reportTitle.${kind}`)} *</h1>
        <Logo />
      </div>
      <div className={`grid grid-cols-1 gap-x-8 gap-y-2 sm:grid-cols-2 print:!grid print:gap-x-4 print:gap-y-0.5 ${open ? "" : "hidden"}`}>
        <Field label={t("header.date")} value={header.date} onChange={(v) => set({ date: v })} type="date" />
        <Field label={t("header.prod")} value={header.prod} onChange={(v) => set({ prod: v })} />
        <Field label={t("header.loueur")} value={header.loueur} onChange={(v) => set({ loueur: v })} />
        <Field label={t("header.film")} value={header.film} onChange={(v) => set({ film: v })} />
        <Field
          label={t("header.assistant")}
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 text-sm font-semibold print:text-[9px]">
          <span className="shrink-0 text-black/70">{t("header.signature")} :</span>
          <MarkableDiagram
            shape="rect"
            dims={SIGNATURE_DIMS}
            printFullSize
